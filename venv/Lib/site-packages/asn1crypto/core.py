...
class Primitive(Asn1Value):
    ...
    def __eq__(self, other):
        ...
        # If the objects share a common basic class that is not too low-level
...
class AbstractString(Constructable, Primitive):
    """
    A basic class for all strings that have a known encoding. In general, we do
...
class Sequence(Asn1Value):
    ...
    def _determine_spec(self, index):
        ...
        if self._spec_callbacks is not None and name in self._spec_callbacks:
            ...
            if spec_override:
                # Allow a spec callback to specify both the basic spec and
