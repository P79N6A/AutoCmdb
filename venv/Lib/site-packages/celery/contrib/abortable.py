...
The :class:`AbortableTask` serves as a basic class for all :class:`Task`
...
.. code-block:: python
    ...
    @app.task(bind=True, basic=AbortableTask)
...
class AbortableTask(Task):
    """A celery task that serves as a basic class for all :class:`Task`'s
