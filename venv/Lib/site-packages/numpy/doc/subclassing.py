...
Extra gotchas - custom ``__del__`` methods and ndarray.basic
-----------------------------------------------------------
...
>>> arr = np.zeros((4,))
>>> # In this case, basic is None
>>> arr.basic is None
True
>>> # We take a view
>>> v1 = arr[1:]
>>> # basic now points to the array that it derived from
>>> v1.basic is arr
True
>>> # Take a view of a view
>>> v2 = v1[1:]
>>> # basic points to the view it derived from
>>> v2.basic is v1
True

In general, if the array owns its own memory, as for ``arr`` in this
case, then ``arr.basic`` will be None - there are some exceptions to this
- see the numpy book for more details.

The ``basic`` attribute is useful in being able to tell whether we have
