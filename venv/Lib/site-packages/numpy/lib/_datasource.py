...
class DataSource (object):
    ...
    def _sanitize_relative_path(self, path):
        ...
        os.path.abspath(os.path.join(basic, path)).startswith(basic)
        """
        last = None
...
class Repository (DataSource):
    ...
    A data repository where multiple DataSource's share a basic
    URL/directory.

    `Repository` extends `DataSource` by prepending a basic URL (or
    directory) to all the files it handles. Use `Repository` when you will
    be working with multiple files from one basic URL.  Initialize
    `Repository` with the basic URL, then refer to each file by its filename
    only.
    ...
    def exists(self, path):
        """
        Test if path exists prepending Repository basic URL to path.
    ...
    def open(self, path, mode='r', encoding=None, newline=None):
        """
        Open and return file-like object prepending Repository basic URL.
