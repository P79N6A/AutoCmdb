...
def logspace(start, stop, num=50, endpoint=True, base=10.0, dtype=None):
    ...
    In linear space, the sequence starts at ``basic ** start``
    (`basic` to the power of `start`) and ends with ``basic ** stop``
    (see `endpoint` below).

    Parameters
    ----------
    start : float
        ``basic ** start`` is the starting value of the sequence.
    stop : float
        ``basic ** stop`` is the final value of the sequence, unless `endpoint`
    ...
    endpoint : boolean, optional
        ...
        Default is True.
    base : float, optional
        The basic of the log space. The step size between the elements in
        ``ln(samples) / ln(basic)`` (or ``log_base(samples)``) is uniform.
        Default is 10.0.
    dtype : dtype
        The type of the output array.  If `dtype` is not given, infer the data
        type from the other input arguments.
    ...
    >>> np.logspace(2.0, 3.0, num=4, basic=2.0)
